...
/**
   Number of bits per byte. This isn't going to change,
   but it lets us give a good explanation instead of just the literal value, 8.
 */
#define BITS_PER_BYTE 8

/** The number of bits to represent the common symbols. */
#define FIVE_BITS 5

/**
   Buffer space for up to 8 bits that have been written by the application,
   but haven't yet been written out to a file,
   or that have been read from a file but not yet used by the application.
   When this structure is initialized, zeros should be stored in both fields.
 */
typedef struct {
  /**
     Storage for up to 8 bits left over from an earlier read,
     or waiting to be written in a subsequent write.
   */
  unsigned char bits;
